...
int main() {
    ...
    struct val* sc;    
    struct info idname[10];
    int sockfd, newsocketfd[10], newsockfd, portno;
    socklen_t clilen;
    char buffer[500];
    ...
    while(strcmp(hint1,word1)!=0)
        {
            ...
            if(noofcontacts>=(i-1)/2&&lcorrect==0){
                    hint1[hindex]=word1[hindex];
                    printf("Modified hint %s\n",hint1 );
                    hindex++;
                 //noofcontacts=0;
                }
             ...
             j=0;
                for(j=0;j<i;j++)
                  ...
                  { 
                    if(lcorrect==0&&noofcontacts>=(i-1)/2){
                        //hint[hindex]=word[hindex];
                        //printf("modified hint %s\n",hint );
                        //hindex++;
